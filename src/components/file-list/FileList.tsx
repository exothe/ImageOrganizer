...
export function FileList({
    ...
    useKeyPressEvent(
        'ArrowDown',
        () => isActive() && selectRow(selectedIndex !== null ? Math.min(selectedIndex + 1, files.length - 1) : null),
    );
    useKeyPressEvent(
        'ArrowUp',
        () => isActive() && selectRow(selectedIndex !== null ? Math.max(selectedIndex - 1, 0) : null),
    );
    useKeyPressEvent('ArrowLeft', () => isActive() && keymap?.left?.(selectedIndex));
    useKeyPressEvent('ArrowRight', () => isActive() && keymap?.right?.(selectedIndex));
    useKeyPressEvent(
        (e) => /^[a-z0-9]$/.test(e.key),
